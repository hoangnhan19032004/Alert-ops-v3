...
namespace AlertOpsBackend.Controllers
{
    ...
    public class AlertsController : ControllerBase
    {
        ...
        public AlertsController(
            ...
            IHubContext<AlertOpsHub> hubContext)
        {
        ...
        public async Task<ActionResult<Alert>> Create(Alert alert)
        {
            ...
            // Tăng alertCount của project nếu alert thuộc project
            if (!string.IsNullOrEmpty(alert.ProjectId))
                _projectService.IncrementAlertCount(alert.ProjectId);
        ...
        public async Task<IActionResult> PatchStatus(
            string id,
            [FromBody] StatusUpdateRequest req)
        {
            var alert = _alertService.Get(id);
            ...
            [FromBody] BulkIdsRequest req)
        {
            ...
            // Lấy danh sách alert trước khi xóa để biết projectId
            var alertsToDelete = req.Ids
                .Select(id => _alertService.Get(id))
                .Where(a => a != null)
                .ToList();

            var deleted = _alertService.BulkDelete(req.Ids);

            // Giảm alertCount theo từng project (group để gọi 1 lần/project)
            var grouped = alertsToDelete
                .Where(a => !string.IsNullOrEmpty(a!.ProjectId))
                .GroupBy(a => a!.ProjectId!);

            foreach (var group in grouped)
                _projectService.DecrementAlertCount(group.Key, group.Count());
            ...
            {
                deleted,
                message = $"{deleted} alert(s) deleted"
            });
        }
            ...
            [FromBody] BulkStatusRequest req)
        {
            ...
            var updated = _alertService.BulkUpdateStatus(
                req.Ids,
                req.Status
            );
            ...
            {
                updated,
                message = $"{updated} alert(s) updated to {req.Status}"
            });
        }
