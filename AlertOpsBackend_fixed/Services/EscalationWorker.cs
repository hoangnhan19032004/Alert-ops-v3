...
namespace AlertOpsBackend.Services
{
    ...
    public class EscalationWorker : BackgroundService
    {
        ...
        // Nếu restart server thì reset — chấp nhận được cho MVP.
        ...
        {
            // Dùng scope vì các service là Scoped
            ...
            {
                // Tính delay thành TimeSpan
                var delay = ParseDelay(rule.Delay);

                // Lấy project để lấy danh sách recipients (owner + managers + members)
                var project = projectService.Get()
                    .FirstOrDefault(p => p.Name == rule.Project ||
                                         p.Id   == rule.Project);
                ...
                foreach (var alert in matchedAlerts)
                {
                    ...
                    try
                    {
                        ...
                        _logger.LogInformation(
                            "Auto-notification sent: rule={Rule}, alert={Alert}, recipients={R}",
                            rule.Id, alert.Id, string.Join(",", recipients));
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex,
                            "Failed to send auto-notification for rule={Rule}, alert={Alert}",
                            rule.Id, alert.Id);
                    }
                }
            }
        ...
        /// <summary>
        /// Phân tích delay string từ UI: "Immediate", "30 sec", "5 min", "1 hour"
        /// </summary>
        ...
        {
            // Thử parse "X min" hoặc "X hour" dạng tùy ý
        ...
        /// <summary>
        /// Alert thuộc project nếu ProjectId khớp với ID hoặc tên project
        /// </summary>
        ...
        /// <summary>
        /// Trigger format từ UI: "critical", "critical × 3", "warning", "5xx", custom text
        /// </summary>
        private static bool MatchesTrigger(Alert alert, string trigger)
        {
            if (string.IsNullOrEmpty(trigger)) return true;

            // Tách phần severity và count: "critical × 3" -> ("critical", 3)
            ...
            // Match theo HTTP status (5xx, 4xx)
            ...
            // Match custom: trigger xuất hiện trong message hoặc service
        ...
        /// <summary>
        /// Lấy danh sách email từ project (owner + managers + members)
        /// Project lưu email dạng string trong Owner, Managers[], Members[]
        /// </summary>
