          ...
          <div v-if="state === 'confirm'" key="confirm" class="card-inner">
            <div class="user-info">
              ...
              <div>
                ...
                <div class="user-meta">{{ currentUser?.email }} · {{ currentUser?.role }}</div>
              </div>
            </div>
...
// FIX #11: cookie cần xóa khi logout
const sessionCookie = useCookie('alertops_session')

// Hiển thị initials thay vì avatar cũ
...
const handleLogout = async () => {
  ...
  // FIX #11: gọi API logout thực (xóa refresh token trên DB)
  await logout()

  // FIX #9: xóa session cookie để middleware SSR không nhận ra nữa
  sessionCookie.value = null

  state.value = 'done'

  // Progress bar rồi redirect
  let p = 0
  timer = setInterval(() => {
    p += 4
    progress.value = p
