...
import type { Project, ProjectSeverity } from '~/types'
...
const loadProject = async () => {
  ...
  try {
    const res = await apiCall<ApiResponse<ProjectApiData> | ProjectApiData>(
      `/api/projects/${route.params.id as string}`
    )
...
// ── FIX: convert email strings → ProjectMember objects trước khi gửi PUT ──
...
const handleSubmit = async () => {
  ...
  saving.value = true
  try {
    await apiCall(`/api/projects/${route.params.id as string}`, {
      method: 'PUT',
      body: {
