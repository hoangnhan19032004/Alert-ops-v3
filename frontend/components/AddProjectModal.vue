...
const handleSubmit = () => {
  ...
  // Convert email strings → ProjectMember objects (backend expects objects, not plain strings)
  const toMember = (email: string, role: string) => {
    ...
    return {
      ...
      email: e,
      role,
      // guard against undefined index access for TypeScript safety
      avatar: (username && username.length > 0) ? username.charAt(0).toUpperCase() : '?'
    }
  }
  ...
  const newProject = {
    ...
    owner: toMember(form.value.owner, 'Owner'),
    managers: form.value.managers.map(e => toMember(e, 'Manager')),
    members:  form.value.members.map(e => toMember(e, 'Member')),
    alertCount: 0
  }
